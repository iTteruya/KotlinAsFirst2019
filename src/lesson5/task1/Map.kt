    ...
    for (i in set) {
        if (frn.containsKey(i)) im.addAll(set + (frn.getValue(i) - remove))
            ...
            if (y.isNotEmpty()) {
                im.addAll(y)
                remove.addAll(y)
                im.addAll(list(friends, frn, y, remove))
            }
        }
    }
    return im
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    ...
    for ((first, second) in stockPrices) {
        if (m.containsKey(first)) m[first]!!.add(second)
        else m[first] = mutableListOf(second)
    }
 ...
 */
fun canBuildFrom(chars: List<Char>, word: String): Boolean = if (word == "") true
else chars.map { it.toLowerCase() }.toSet().union(word.toLowerCase().toSet()) ==
        chars.map { it.toLowerCase() }.toSet()
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    for (i in list.indices) {
        ...
        if (m.containsKey(num) && m[num] != i)
            return Pair(m[num]!!, i).sorted()
    }
    return Pair(-1, -1)
}
