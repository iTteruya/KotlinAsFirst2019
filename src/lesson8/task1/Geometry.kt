...
data class Circle(val center: Point, val radius: Double) {
    ...
    fun contains(p: Point): Boolean = center.distance(p) - radius <= 1e-16
}
...
fun diameter(vararg points: Point): Segment {
    ...
    for (i in 0 until points.size - 1) {
        for (ii in i + 1 until points.size) {
            val dis = points[i].distance(points[ii])
            if (dis >= max.begin.distance(max.end)) max = Segment(points[i], points[ii])
        }
    }
    return max
}
    ...
    for (i in 0 until circles.size - 1) {
        for (ii in i + 1 until circles.size) {
            if (abs(circles[i].distance(circles[ii])) < ml) {
                ml = circles[i].distance(circles[ii])
                np = Pair(circles[i], circles[ii])
            }
        }
    }
    return np
...
fun minContainingCircle(vararg points: Point): Circle {
    ...
    val cp = mutableListOf(points[0], points[1])
